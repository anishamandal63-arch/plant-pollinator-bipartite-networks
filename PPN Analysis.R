...
mat1[is.na(mat1)] <- 0
################################################ degree; species strength; nestedness contribution
specieslevel (mat1)
specieslevel (mat)

set.seed(123)

result_nc_mat1 <- nestedcontribution(mat1)
result_nc_mat  <- nestedcontribution(mat)
#########################insect orders
...
order_richness <- nectar %>%
  filter(!is.na(Scientificn)) %>%
  group_by(FT, Insect.order) %>%
  summarise(
    Species_richness = n_distinct(Scientificn),
    .groups = "drop"
  ) %>%
  arrange(FT, desc(Species_richness))

order_richness
## interaction frequency
order_interactions <- nectar %>%
  group_by(FT, Insect.order) %>%
  summarise(
    Total_interactions = sum(PN, na.rm = TRUE),
    .groups = "drop"
  ) %>%
  arrange(FT, desc(Total_interactions))
...
par(mfrow = c(1,2), mar = c(8,10,4,2))
...
#######################################################################################
############    Seasonality
# Bootstrap NODF & Modularity (Habitat × Season) — 1000 iterations per group
##############################################################################
...
# 2. Keep only nectaring interactions
nectar_data <- droplevels(subset(pp.data, Activity == "Nectaring"))

# 3. Matrix builder (keeps all species)
make_matrix <- function(data, plant_col = "Plants", poll_col = "Scientificn", count_col = "PN") {
  plants <- sort(unique(na.omit(data[[plant_col]])))
  polls  <- sort(unique(na.omit(data[[poll_col]])))
  if (length(plants) == 0 || length(polls) == 0) return(NULL)
  mat <- matrix(0, nrow = length(plants), ncol = length(polls),
                dimnames = list(plants, polls))
  for (i in seq_along(plants)) {
    for (j in seq_along(polls)) {
      temp <- subset(data, data[[plant_col]] == plants[i] & data[[poll_col]] == polls[j])
      mat[i, j] <- sum(temp[[count_col]], na.rm = TRUE)
    }
  }
  mat[is.na(mat)] <- 0
  return(mat)
...
# 4. Bootstrap function for one group (data subset)
bootstrap_group <- function(data_subset, n_iter = 1000, seed = 123, progress_every = 100) {
  if (nrow(data_subset) == 0) return(NULL)
  set.seed(seed)
  nodf_vals <- numeric(n_iter)
  mod_vals  <- numeric(n_iter)
  for (i in 1:n_iter) {
    samp <- data_subset[sample(1:nrow(data_subset), nrow(data_subset), replace = TRUE), ]
    mat  <- make_matrix(samp)
    ...
    nodf_vals[i] <- tryCatch(networklevel(mat_bin, index = "NODF"), error = function(e) NA)
    mod_vals[i]  <- tryCatch(computeModules(mat)@likelihood, error = function(e) NA)
    if (!is.na(progress_every) && (i %% progress_every == 0)) cat("  iter:", i, "\n")
  }
  list(NODF = nodf_vals, Mod = mod_vals)
}

# 5. Define groups
groups <- expand.grid(Habitat = c("PF", "IDH"),
                      Season  = c("Pre-monsoon", "Post-monsoon"),
                      stringsAsFactors = FALSE) %>%
  arrange(Habitat, Season)

# 6. Run bootstrap per group and compute observed + CI
results_list <- vector("list", nrow(groups))
names(results_list) <- paste(groups$Habitat, groups$Season, sep = "_")

for (k in seq_len(nrow(groups))) {
  ft <- groups$Habitat[k]
  season <- groups$Season[k]
  cat("\n==== Processing:", ft, "|", season, "====\n")
  sub <- droplevels(subset(nectar_data, FT == ft & Season == season))
  
  # Observed matrix & metrics
  mat_obs <- make_matrix(sub)
  if (is.null(mat_obs)) {
    cat(" No data for", ft, season, "\n")
    results_list[[k]] <- list(summary = NULL, nulls = NULL)
    next
  }
  mat_obs_bin <- (mat_obs > 0) * 1
  obs_nodf <- tryCatch(networklevel(mat_obs_bin, index = "NODF"), error = function(e) NA)
  obs_mod  <- tryCatch(computeModules(mat_obs)@likelihood, error = function(e) NA)
  
  # Bootstrap
  nulls <- bootstrap_group(sub, n_iter = n_iter, seed = seed + k)  # different seed per group
  nodf_vals <- nulls$NODF
  mod_vals  <- nulls$Mod
  
  # Clean NAs
  nodf_vals_f <- nodf_vals[is.finite(nodf_vals)]
  mod_vals_f  <- mod_vals[is.finite(mod_vals)]
  
  ci_nodf <- if (length(nodf_vals_f) > 0) quantile(nodf_vals_f, probs = c(0.025, 0.975)) else c(NA, NA)
  ci_mod  <- if (length(mod_vals_f) > 0) quantile(mod_vals_f,  probs = c(0.025, 0.975)) else c(NA, NA)
  
  summary_df <- data.frame(
    Habitat = ft,
    Season = season,
    NODF_obs = obs_nodf,
    NODF_mean = ifelse(length(nodf_vals_f)>0, mean(nodf_vals_f), NA),
    NODF_lower = ci_nodf[1], NODF_upper = ci_nodf[2],
    Modularity_obs = obs_mod,
    Modularity_mean = ifelse(length(mod_vals_f)>0, mean(mod_vals_f), NA),
    Modularity_lower = ci_mod[1], Modularity_upper = ci_mod[2],
    Replicates_used_NODF = length(nodf_vals_f),
    Replicates_used_Mod  = length(mod_vals_f),
    stringsAsFactors = FALSE
  )
  
  results_list[[k]] <- list(summary = summary_df, nulls = list(NODF = nodf_vals_f, Mod = mod_vals_f))
  cat(" Done:", ft, season, "- valid NODF reps:", length(nodf_vals_f),
      "valid Mod reps:", length(mod_vals_f), "\n")
}

# 7. Combine summaries
summaries <- do.call(rbind, lapply(results_list, function(x) x$summary))
print(summaries)
...
# Observed values from original networks
obs_PF_nodf <- networklevel((mat1 > 0) * 1, index = "NODF")
obs_IDH_nodf <- networklevel((mat > 0) * 1, index = "NODF")

obs_PF_mod <- computeModules(mat1)@likelihood
obs_IDH_mod <- computeModules(mat)@likelihood

results_df <- data.frame(
  Habitat = c("PF", "IDH"),
  
  NODF_mean = c(results_PF$NODF_mean,
                results_IDH$NODF_mean),
  
  NODF_lower = c(results_PF$NODF_CI[1],
                 results_IDH$NODF_CI[1]),
  
  NODF_upper = c(results_PF$NODF_CI[2],
                 results_IDH$NODF_CI[2]),
  
  NODF_obs = c(obs_PF_nodf,
               obs_IDH_nodf),
  
  Modularity_mean = c(results_PF$Mod_mean,
                      results_IDH$Mod_mean),
  
  Modularity_lower = c(results_PF$Mod_CI[1],
                       results_IDH$Mod_CI[1]),
  
  Modularity_upper = c(results_PF$Mod_CI[2],
                       results_IDH$Mod_CI[2]),
  
  Modularity_obs = c(obs_PF_mod,
                     obs_IDH_mod)
)
################## plot Q
p_mod <- ggplot(results_df,
                aes(x = Habitat,
                    y = Modularity_mean,
                    color = Habitat)) +
  ...
  geom_errorbar(aes(ymin = Modularity_lower,
                    ymax = Modularity_upper),
                width = 0.12,
                linewidth = 0.8) +
  
  geom_point(aes(y = Modularity_obs),
             shape = 17,
             size = 3,
             color = "black") +
  ...
  labs(y = "Modularity (Q)",
       x = "",
       title = "Bootstrap estimates of modularity") +
...
p_nodf <- ggplot(results_df,
  ...
  geom_errorbar(aes(ymin = NODF_lower,
                    ymax = NODF_upper),
                width = 0.12,
                linewidth = 0.8) +
  
  geom_point(aes(y = NODF_obs),
             shape = 17,
             size = 3,
             color = "black") +
  
  labs(y = "Nestedness (NODF)",
       x = "",
       title = "Bootstrap estimates of nestedness") +
...
# PF
pf_links <- with(
  subset(nectar_data, FT == "PF"),
  paste(Plants, Scientificn, sep = "_")
)

estimateR(table(pf_links))

# IDH
idh_links <- with(
  subset(nectar_data, FT == "IDH"),
  paste(Plants, Scientificn, sep = "_")
)

estimateR(table(idh_links))


pf_est <- estimateR(table(pf_links))

PF_completeness <-
  100 * pf_est["S.obs"] / pf_est["S.chao1"]

PF_completeness
idh_est <- estimateR(table(idh_links))

IDH_completeness <-
  100 * idh_est["S.obs"] / idh_est["S.chao1"]
...
PF_abund <- as.numeric(table(pf_links))
IDH_abund <- as.numeric(table(idh_links))
...
cat("IDH total interaction events :", sum(mat), "\n")
cat("IDH unique interactions      :", sum(mat > 0), "\n\n")

cat("PF total interaction events  :", sum(mat1), "\n")
cat("PF unique interactions       :", sum(mat1 > 0), "\n")
